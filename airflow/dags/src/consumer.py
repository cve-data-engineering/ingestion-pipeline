...
class MessageConsumer:
    ...
    def consume_and_embed_messages(self, flag, inactivity_timeout=10):
        """Consume messages until inactivity timeout is reached"""
        ...
        while True:
            ...
            if message_batch:
                last_message_time = time.time()  # Reset the timer since we received messages

                for topic_partition, messages in message_batch.items():
                    for message in messages:
                        if not flag:
                            print("Control to upload to Snowflake")
                            self.snowflake_uploader.upload_json_to_snowflake(message.value)
                        else:
                            print("Control to process and store embedding")
            ...
            if (time.time() - last_message_time) > inactivity_timeout:
                logging.info("Inactivity timeout reached. Exiting consumer loop.")
                break
